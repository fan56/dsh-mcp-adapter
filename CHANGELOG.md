...
- **旧值承接**：entry id 保持 `dsh-mcp-adapter` 不变——旧 profile patch 里该 entry `config:` 段的 prefix/keep/servers 配置继续生效；但旧 settings.yaml 的 `mcp-adapter:` 段（旧 settings 门闩状态）在 0.1.7 一次性导入时因段名 ≠ entry id 不会自动迁移（原文保留在 `settings.yaml.imported`），此前 disable 过的 server 需各补一条 `/mcp disable <id>`。
